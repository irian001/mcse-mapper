...
import { Save, Search, Plus, X, Pencil, Filter } from "lucide-react";
...
const emptyForm = {
  ...
  descricao_mcse: "",
  conta_critica: false,
  exige_documento_obrigatorio: false,
  exige_revisao_humana: false,
  exige_conciliacao_reg_soc: false,
  materialidade_padrao: "",
  limite_variacao_percentual: "",
  limite_variacao_absoluta: "",
  grupo_documental: "",
  gera_solicitacao_automatica: false,
  ativo: true,
  observacao_regra: "",
};
...
export default function RegrasPage() {
  ...
  // Contas que já têm regra (para evitar duplicidade no select)
  ...
  const openEdit = (r: RegraRow) => {
    ...
    setForm({
      ...
      codigo_mcse: r.codigo_mcse || "",
      descricao_mcse: r.descricao_mcse || "",
      conta_critica: r.conta_critica,
      exige_documento_obrigatorio: r.exige_documento_obrigatorio,
      exige_revisao_humana: r.exige_revisao_humana,
      exige_conciliacao_reg_soc: r.exige_conciliacao_reg_soc,
      ...
      grupo_documental: r.grupo_documental || "",
      gera_solicitacao_automatica: r.gera_solicitacao_automatica,
      ativo: r.ativo,
  ...
  const handleSelectConta = (contaId: string) => {
    ...
    if (conta) {
      setForm((f) => ({
        ...
        descricao_mcse: conta.descricao_conta,
        conta_critica: conta.conta_critica || false,
      }));
    }
  };
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      const payload = {
        conta_mcse_id: form.conta_mcse_id,
        codigo_mcse: form.codigo_mcse || null,
        descricao_mcse: form.descricao_mcse || null,
        conta_critica: form.conta_critica,
        ...
        grupo_documental: form.grupo_documental || null,
        gera_solicitacao_automatica: form.gera_solicitacao_automatica,
        ativo: form.ativo,
    ...
    onError: (e: any) => {
      if (e?.message?.includes("unique") || e?.code === "23505") {
        toast.error("Já existe uma regra para esta conta MCSE.");
      } else {
  ...
  const filtered = regras.filter((r) => {
    if (search) {
      ...
      if (!(r.codigo_mcse || "").toLowerCase().includes(s) && !(r.descricao_mcse || "").toLowerCase().includes(s)) return false;
    }
  ...
  return (
    <div>
      <PageHeader title="Regras MCSE" description="Regras de auditoria por conta MCSE — tabela-mãe documental" />
          ...
          <CardContent className="space-y-5">
            ...
            {!editing && (
              <div>
                ...
                <Select value={form.conta_mcse_id} onValueChange={handleSelectConta}>
                  ...
                  <SelectContent>
                    {contas
                      .filter((c: any) => !contasComRegra.has(c.id))
                      .map((c: any) => (
                        <SelectItem key={c.id} value={c.id}>
                          <span className="font-mono text-xs mr-2">{c.codigo_mcse}</span>
                          {c.descricao_conta}
                        </SelectItem>
                      ))}
                  </SelectContent>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <label className="flex items-center gap-2 text-sm">
                <Checkbox checked={form.conta_critica} onCheckedChange={(v) => setForm((f) => ({ ...f, conta_critica: !!v }))} />
                Conta Crítica
              </label>
              <label className="flex items-center gap-2 text-sm">
                <Checkbox checked={form.exige_documento_obrigatorio} onCheckedChange={(v) => setForm((f) => ({ ...f, exige_documento_obrigatorio: !!v }))} />
                Documento Obrigatório
              </label>
              <label className="flex items-center gap-2 text-sm">
                <Checkbox checked={form.exige_revisao_humana} onCheckedChange={(v) => setForm((f) => ({ ...f, exige_revisao_humana: !!v }))} />
                Revisão Humana
              </label>
              <label className="flex items-center gap-2 text-sm">
                <Checkbox checked={form.exige_conciliacao_reg_soc} onCheckedChange={(v) => setForm((f) => ({ ...f, exige_conciliacao_reg_soc: !!v }))} />
                Conciliação REG/SOC
              </label>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Select value={form.grupo_documental} onValueChange={(v) => setForm((f) => ({ ...f, grupo_documental: v }))}>
                  ...
                  <SelectContent>
                    {GRUPOS_DOCUMENTAIS.map((g) => (
                      <SelectItem key={g} value={g}>{grupoLabel[g]}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              <div className="flex flex-col justify-end gap-2">
                <label className="flex items-center gap-2 text-sm">
                  <Checkbox checked={form.gera_solicitacao_automatica} onCheckedChange={(v) => setForm((f) => ({ ...f, gera_solicitacao_automatica: !!v }))} />
          ...
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            <Select value={filterCritica} onValueChange={setFilterCritica}>
              ...
              <SelectContent>
                ...
                <SelectItem value="sim">Crítica</SelectItem>
                <SelectItem value="nao">Não crítica</SelectItem>
              </SelectContent>
            </Select>
            <Select value={filterSolicitacao} onValueChange={setFilterSolicitacao}>
              <SelectTrigger className="w-[170px]"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="all">Solicitação</SelectItem>
                <SelectItem value="sim">Gera solic.</SelectItem>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-center">Doc. Obrig.</TableHead>
                  <TableHead className="text-center">Rev. Humana</TableHead>
                  <TableHead className="text-center">Conc. REG/SOC</TableHead>
                  <TableHead>Grupo Doc.</TableHead>
                  <TableHead className="text-center">Solic. Auto</TableHead>
                ...
                {filtered.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={10} className="text-center text-muted-foreground py-12">
                      {isLoading ? "Carregando..." : "Nenhuma regra encontrada"}
                    </TableCell>
                  </TableRow>
                )}
                {filtered.map((r) => (
