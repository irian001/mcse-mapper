...
export default function ContagemEstoqueBlocoDetail({ bloco, open, onClose }: Props) {
  ...
  const totais = useMemo(() => {
    ...
    return { sis, cnt, dif, withDif, count: itens.length };
  }, [itens]);
      ...
      <DialogContent className="max-w-6xl max-h-[92vh] overflow-hidden flex flex-col p-0">
        <DialogHeader className="px-6 pt-5 pb-3 border-b border-border">
          <DialogTitle className="text-base">
            Itens do Bloco — {bloco?.filial || "—"} · {bloco?.setor || "—"} · {bloco?.tipo_estoque || "—"}
          </DialogTitle>
          <div className="flex flex-wrap gap-2 mt-2 text-xs text-muted-foreground">
            ...
            {totais.withDif > 0 && <span className="text-warning-foreground">· {totais.withDif} com divergência</span>}
          </div>
        </DialogHeader>
          ...
          <div className="border border-border rounded-lg p-3 bg-muted/20">
            ...
            <div className="grid grid-cols-12 gap-2">
              <div className="col-span-2">
                ...
                <Input
                  value={novo.codigo_item}
                  onChange={(e) => setNovo({ ...novo, codigo_item: e.target.value })}
                />
              </div>
            ...
            <Table>
              ...
              <TableBody>
                ...
                {!isLoading && itens.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={10} className="text-center text-muted-foreground py-6">
                      Nenhum item registrado neste bloco.
                    </TableCell>
                  </TableRow>
                )}
                {itens.map((it: any) => (
...
function ItemRow({
  ...
  return (
    <TableRow>
      <TableCell className="text-sm font-mono">{item.codigo_item || "—"}</TableCell>
      <TableCell className="text-sm">{item.descricao_item || "—"}</TableCell>
      <TableCell className="text-sm">{item.unidade_medida || "—"}</TableCell>
